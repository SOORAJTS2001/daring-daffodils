...
function startHttpPolling() {
    ...
    pollingInterval = setInterval(async () => {
        try {
            console.log("Background: Attempting to fetch from", HTTP_POLLING_URL);
            ...
            if (response.ok) {
                ...
                if (data && Object.keys(data).length > 0) {
                    console.log("Background: Sending data to content scripts");
                    // Send data to all content scripts
                    chrome.tabs.query({}, (tabs) => {
                        tabs.forEach(tab => {
                            chrome.tabs.sendMessage(tab.id, {
                                ...
                                console.log("Background: Error sending to tab", tab.id, error);
                            });
                        });
                    });
                } else {
                    console.log("Background: No data available (empty response)");
                }
            } else {
                console.error("Background: HTTP response not OK:", response.status, response.statusText);
            }
        } catch (error) {
            console.error("Background: HTTP polling error:", error);
        }
    }, 500); // Slower polling for debugging
}
            }
        } catch (error) {
            console.error("Background: HTTP polling error:", error);
